...
if(isset($_POST['form1'])){
	...
	$query = $pdo->prepare("UPDATE site 
		                    ...
		                    webmail =?
		                    WHERE id = ?");
	$query->execute(array($_POST['client'],$_POST['license'],$_POST['ssl'],$_POST['domaine'],$_POST['mail'],$_GET['id']));
  ...
  if($certificat != $_POST['ssl']){
	  ...
	  $query->execute(array($_GET['id'],$_POST['ssl']));
		...
		header('Location:site_active.php');
				...
				<div class="">
					<div class="page-title">
						<div class="title_left">
							<h3>Edit du Site Web <small>"<?php echo $domaine; ?>"</small></h3>
						</div>
					</div>
								...
								<div class="x_content">
									...
									<form method="post">
										...
										<div class="form-group row">
											<label class="col-form-label col-md-3 col-sm-3 label-align" for="client">Client *</label>
											<div class="col-md-6 col-sm-6 ">
												...
												<?php
													...
													foreach ($result as $row) {
													 ?>
													<option value="<?php echo $row['id']; ?>" <?php if($row['id']==$client){ echo 'selected';} ?> ><?php echo $row['fullname']; ?></option>
												<?php } ?>
												</select>
											</div>
										</div>
